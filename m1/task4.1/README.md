## create db cars
![alt text](/m1/task4.1/screenshots/1.png) 
## use db cars
![alt text](/m1/task4.1/screenshots/2.png) 
## insert data
![alt text](/m1/task4.1/screenshots/3.png) 
## SELECT WHERE
![alt text](/m1/task4.1/screenshots/4.png) 
## SELECT GROUP
![alt text](/m1/task4.1/screenshots/5.png) 
## SELECT ORDER
![alt text](/m1/task4.1/screenshots/6.png) 
## create table by ddl
![alt text](/m1/task4.1/screenshots/7.png) 
## INSERT INTO table dml
![alt text](/m1/task4.1/screenshots/8.png) 
## create user and add all proviliges
![alt text](/m1/task4.1/screenshots/9.png) 
## user user can add to table
![alt text](/m1/task4.1/screenshots/10.png) 
## create user1 with with root access
![alt text](/m1/task4.1/screenshots/11.png) 
## user can create not change or add
![alt text](/m1/task4.1/screenshots/12.png) 
## add user2 inserting to databese cars and tables customer
![alt text](/m1/task4.1/screenshots/13.png) 
## user2 dont have acces to table delivery
![alt text](/m1/task4.1/screenshots/14.png) 
## Make a selection from the main table DB MySQL
![alt text](/m1/task4.1/screenshots/15.png) 
## make backup db cars
![alt text](/m1/task4.1/screenshots/www.png) 
## delete customer table
![alt text](/m1/task4.1/screenshots/17.png) 
## restore chack
![alt text](/m1/task4.1/screenshots/18.png) 
## create mysql rds
![alt text](/m1/task4.1/screenshots/19.png) 
## conect to mysql rds , create same user as ta local host
![alt text](/m1/task4.1/screenshots/20.png) 
## at aws rds create data base , add priviliges user to db cars
![alt text](/m1/task4.1/screenshots/21.png) 
## transfer db cars
![alt text](/m1/task4.1/screenshots/22.png) 
## conect to rds aws and check db cars
![alt text](/m1/task4.1/screenshots/23.png) 
## craate snapshot
![alt text](/m1/task4.1/screenshots/24.png) 
## add data to mongo db table
![alt text](/m1/task4.1/screenshots/25.png) 
## Scan dynamo db
![alt text](/m1/task4.1/screenshots/26.png) 
## query dynamo db
![alt text](/m1/task4.1/screenshots/27.png) 

